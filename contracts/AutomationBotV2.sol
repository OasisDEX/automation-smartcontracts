...
contract AutomationBotV2 {
    ...
    function addTriggers(
        ...
        for (uint256 i = 0; i < triggerData.length; i++) {
            ...
            AutomationBotV2(automationBot).addRecord(
                triggerTypes[i],
                continuous[i],
                replacedTriggerId[i],
                triggerData[i]
        ...
        AutomationBotV2(automationBot).emitGroupDetails(groupType, triggerIds);
    }
    ...
    function removeTrigger(uint256 triggerId, bytes memory triggerData) private {
        address automationBot = serviceRegistry.getRegisteredService(AUTOMATION_BOT_KEY);
        BotV2Like(automationBot).removeRecord(triggerData, triggerId);
    }
